...
# File extensions for which XLM / Excel 4.0 macro scanning is applicable.
# XLMMacroDeobfuscator only understands Excel container formats — running it
# on Word or PowerPoint files would raise an exception or return no results.
EXCEL_FORMATS = {".xlsm", ".xls", ".xlsb", ".xltm"}
...
class DocumentScanner:
    ...
    def analyze(self) -> dict:
        ...
        # Only runs on Excel formats — XLM macros are an Excel-specific
        # feature and XLMMacroDeobfuscator does not handle Word/PowerPoint.
        # ----------------------------------------------------------------
        file_ext = os.path.splitext(self.file_path)[1].lower()
        if XLM_AVAILABLE and file_ext in EXCEL_FORMATS:
            try:
